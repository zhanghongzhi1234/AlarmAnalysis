...
namespace AlarmAnalysisService
{
    public class SQLiteServer : DatabaseServer
    {
        ...
        public SQLiteServer(string name, string databaseName = "data.db")
        {
            ...
            DebugUtil.Instance.LOG.Debug("SQLiteServer name=" + name + ", databaseName=" + databaseName);
            //ConnectToDatabase(databaseName);
        }

        public override void Init()
        {
            DebugUtil.Instance.LOG.Info("Function Enter: Init");
            ConnectToDatabase(databaseName);
            DebugUtil.Instance.LOG.Info("Function Exit: Init");
        }
        ...
        public void ConnectToDatabase(string databaseName)
        {
            ...
            try
            {
                ...
                DebugUtil.Instance.LOG.Debug("connString=" + connString);
                mySQLiteConn.Open();
            }
            catch (Exception ex)
            {
                Console.WriteLine("Failed to create a database connection. \n{0}", ex.Message);
                DebugUtil.Instance.LOG.Error("Error: Failed to create a database connection. reason=" + ex.Message);
                return;
            }
        }
        ...
        public override void Close()
        {
            if (mySQLiteConn != null && mySQLiteConn.State == ConnectionState.Open)
            {
                ...
                {
                    string errorMsg = "Error: Failed to close a database connection. \n" + ex.Message;
                    DebugUtil.Instance.LOG.Error(errorMsg);
                    return;
                }
        ...
        public override int ExecuteNonQuery(string sqlstr)
        {
            ...
            {
                string errorMsg = "Error: Failed to execute non query: " + sqlstr + ". \n" + ex.Message;
        ...
        public int ExecuteNonQuery(string[] sqlstrArray, bool bUseTransaction = false)
        {
            //log.Debug(sqlstr);
            if (sqlstrArray == null || sqlstrArray.Count() == 0)
                return 0;
            int ret = 0;
            SQLiteCommand command = new SQLiteCommand(mySQLiteConn);
            DbTransaction trans = null;
            if(bUseTransaction == true)
                mySQLiteConn.BeginTransaction();
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                string errorMsg = "Error: Failed to execute non query: " + command.CommandText + ". \n" + ex.Message;
                DebugUtil.Instance.LOG.Error(errorMsg);
                ret = -1;
                throw (ex);
            }
        ...
        //update DataTable to Datatable, if updatedColuumnNames is null then update all columns
        public int UpdateDatatableToDb(string tableName, DataTable dtContent, string[] updatedColumnNames = null)
        {   //only update close time
            return ExecuteNonQuery(CreateUpdateStatement(tableName, dtContent, "alarmID", updatedColumnNames).ToArray());
        }
