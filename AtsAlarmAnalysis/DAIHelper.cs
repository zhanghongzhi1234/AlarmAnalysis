...
namespace TemplateProject
{
    public sealed class DAIHelper
    {
        ...
        public List<AtsAlarm> GetTodayAtsAlarmListBySystemKeyFromDb(int systemkey, string orderbyTime = "asc")
        {
        ...
        public List<AtsAlarm> GetTodayAtsAlarmListAll(string orderbyTime = "asc")
        {
            string sqlstr = "select * from alarm where 1=1" + CreateSQLForToday();
            sqlstr += " order by sourceTime " + orderbyTime;
            return GetAtsAlarmsFromDb(sqlstr);
        ...
        //IsAtsAlarm: 1 mean AtsAlarm, 0 mean IscsAlarm
        public List<AtsAlarm> GetAlarmListBetween(DateTime startTime, DateTime endTime, int IsAtsAlarm = 1, string tableName = "Alarm", string locationId = "", List<string> subsystemkeyList = null, string orderbyTime = "asc")
        {
        ...
        public DataTable GetTop10AlarmTableFromDb(string strCondition, string strRule, int IsAtsAlarm = 1)
        {
            ...
            if (IsAtsAlarm == 1)
            {
                strColumns = "alarmDescription, alarmSeverity, locationId, assetName, count(*) as count";
            }
            else
            {
                strColumns = "alarmSeverity, locationId, alarmDescription, count(*) as count";
            }
            string sqlstr = "select " + strColumns + " from alarm where IsAtsAlarm=" + IsAtsAlarm.ToString() + strCondition + " group by " + strRule + " order by count desc limit 0,10";
            ...
            if (schemaTable != null)
            {
                ...
                foreach (DataRow myField in schemaTable.Rows)
                {
                    ...
                    if (ackTime != null && ackTime != "" && ackTime != "0")
                        ...
                        item.Ack = "0";
                    /*try           //closeTime is not use anymore
                    {
                        item.closeTime = Convert.ToInt32(myField["closeTime"].ToString());
                    }
                    catch (Exception ex)
                    {
                    ...
                    if (IsAtsAlarm == 1)
                    {
                        ...
                        item.subsystemID = AtsCachedMap.Instance.GetSubsystemIDByName(myField["subsystemType"].ToString(), item.systemID);
                    }
                    else
                    {
